			...
			if(mysqli_num_rows($result)==1){
				...
				<div class='w3-card-4'>
					<header class='w3-container w3-green'>
						...
						asked by <a style='color:black' href='profile.php?user_id=$row[user_id]'>$row[username]</a> 
						at $row[time]
					</header>
					<br>
				...
				if(isset($_SESSION['loggedin']) && $_SESSION['loggedin']==true){
					echo 
					"
					<button class='btn-success' onclick='load();'>
						<span class='glyphicon glyphicon-thumbs-up'></span>
						<b> Like </b> 
						<span id='like' class='badge'>$vote</span>
					</button>
					";
				}
				else{
					echo 
					"
					<button class='btn-success' onclick='show();'>
						<span class='glyphicon glyphicon-thumbs-up'></span>
						<b> Like </b> 
						<span id='like' class='badge'>$vote</span>
					</button>
...
<?php
		...
		if(mysqli_num_rows($answer_res)>0){
			...
			while($answer_row=mysqli_fetch_assoc($answer_res)){
				...
				echo 
				"
				<hr><p><b>$answer_row[answer]</b>
				<span class='badge pull-right'>Answered at $answer_row[time]</span>
				<br><i>.......answered by </i>
				...
				if(isset($_SESSION['loggedin']) && $_SESSION['loggedin']==true){
					echo 
					"
					<button class='btn-success' onclick='answer_load($answer_row[id],$answer_vote);'>
						<span class='glyphicon glyphicon-thumbs-up'></span>
						<b> Like </b> 
						<span id='answer_like' class='badge'>$answer_vote</span>
					</button>
					";	
				}
				else{
